...
Items in a Python list are ordered, meaning we can refer to them by their index (position) in the list. Python uses zero-based indexing, so the first element occupies the 0th position. If a list has 5 elements, the last element will be in the 4th position. Use square brackets to access a value at a given position.
...
To traverse all the items in a list and perform operations on them, Python offers `for` and `while` loops. For example, if we want to add 1000 to each revenue, a manual approach is error-prone and not scalable. Instead, we can use a loop to programmatically add 1000 to each item in the `revenues` list.
...
The `rev` variable in the `for` loop can be any valid Python variable. It acts as a placeholder to traverse all the items in the `revenues` list. For each item, we add 1000 and append the result to our `result` list.
...
Rewrite the `for` loop above using a counter, similar to the `while` loop approach.
