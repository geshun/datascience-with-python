...
Let's put together everything we've learned to write our own function for computing the arithmetic mean. Functions in Python are defined using the `def` keyword. We'll also include a brief docstring to describe the function. You can see the docstring by using `help(arithmetic_mean)`. Functions help encapsulate code, hiding the implementation details and making the code easier to reuse and maintain. For example, instead of writing `sum(x) / len(x)` every time, we can encapsulate it into a function.
...
We can also try this with recursion. Recursion involves solving a problem by breaking it down into simpler versions of the same problem.
...
def product_(x):
    if len(x) == 1:
        ...
        return x[0] * product_(x[1:])

product_(revenues) == product(revenues)

```

In the recursive approach, we first check if we are looking at a single-element list. If so, that number is returned as the product. Otherwise, we re-write our problem as the first element times our function called on the remaining elements. This process continues until we arrive at a single-element list, which is our base case.

If `return` statements end execution, how does recursive computation continue after the first return statement is reached? Suppose we have a list `x = [2, 3]`. Since the length is greater than 1, our `product_()` function will return `2 * product_([3])`. This returns a call to a function, so that function must be executed. The result `2 * product_([3])` is kept in a call stack. The `product_([3])` is executed, and since its length is 1, it returns 3. The `return` statement then replaces `product_([3])` with 3, resulting in `2 * 3 = 6`.
...
Finally, we'll write a function for computing the harmonic mean. The harmonic mean involves inverting the elements of a list, so we don't need a separate function for that step.
...
```{python}
print(harmonic_mean(revenues) <= geometric_mean(revenues) <= arithmetic_mean(revenues))
```
