...
Data science is not just about working with data; it starts with a problem and the thinking process behind it. Organizations hire data scientists because they at least believe they have problems that data can address. At its core, data science is about problem-solving. While the tools and techniques used in data science evolve over time, the fundamental problem-solving skills remain the same.
...
Each phase of data science can be viewed as a problem-solving task. Defining the problem is the first challenge, as solving the wrong problem is a waste of time and resources. Problem definition determines what data needs to be collected. Collecting the relevant data of good quality in the appropriate amount is another problem that requires problem-solving skills. Fundamental to a rich insight is the quantity, relevance and quality of the data used.

Analyzing the data to generate insights involves identifying patterns, trends, and relationships. For instance, a data scientist might analyze customer behavior to determine which products are frequently purchased together and use this insight for targeted marketing campaigns.
...
Let's put together everything we've learned to write our own function for computing the arithmetic mean. Functions in Python are defined using the `def` keyword. We'll also include a brief `docstring` to describe what the function does. This is written in three quotation marks `""" write docstring here"""`.

Functions help encapsulate code, hiding the implementation details and making the code easier to reuse and maintain. For example, instead of writing `sum(x) / len(x)` every time we want the arithmetic mean, we can encapsulate it into a function.
...
You can see the `docstring` by using the `help()` function on the `arithemtic_mean`.
...
Now, let's move on to the geometric mean, which requires multiplying the items in our list. First, we'll write a function that computes the product of a list. This function will be useful not only for computing the geometric mean but also in other contexts where we need to find the product of numbers.

```{python}
#| eval: false

def product(x):
    """Finds the product of values in a list"""
    result = 1
    for val in x:
        result *= val
    return result
...
def geometric_mean(x):
    """Finds the geometric mean of a list of numbers"""
...
Finally, we'll write a function for computing the harmonic mean. The harmonic mean involves inverting the elements of a list, however, we will not write a separate function for that step.

```{python}
def harmonic_mean(x):
    """Finds the harmonic mean of a list of numbers"""
...
To save your code for reuse, put all these functions in a Python file. A Python file should have the extension `.py`.

#### Example Python File: `means.py`
...
if __name__ == "__main__":
    revenues = [12000, 23000, 15000, 10000, 18000]
    print("Arithmetic Mean:", arithmetic_mean(revenues))
    print("Geometric Mean:", geometric_mean(revenues))
    print("Harmonic Mean:", harmonic_mean(revenues))
```

#### Running the Python File

Save the file as `means.py` and run it from the terminal:

```{bash}
python means.py
```

This approach shows how to save and reuse your code, making it easy to maintain and share.

## Linear Models

A model is a simplified representation of the real world.

Consider the company, $KN^4$, that uses Facebook ads as part of its marketing strategy. The company wants to find a way to increase revenue while managing the cost of advertisements. If we can establish a relationship between ad cost and revenue i.e.,

$$
revenue = f(ad\:cost) + \epsilon
$$the company can optimize its ad spending to achieve its objectives. Here, $f(ad\:cost)$ represents the function that estimates revenue based on ad cost and the goal is to determine what this function $f$ is. Since our model is an approximation (it's not perfect), we include an error term $\epsilon$ to account for the differences between the model's predictions and the actual revenue. The form of $f$ determines whether the model is linear or not.

### Linear Models and Their General Form

If the relationship between ad cost and revenue is linear, our model will take a linear form. A linear model generally expresses the predicted value $\hat{y}_i$ as a weighted sum of the input features (in this case, the ad cost):

$$
\hat{y}_i = \sum_{j=1}^{n}w_jx_{ij} = w_1x_{i1} + \dots + w_nx_{in}
$$

-   $n$ is the number of features (independent variables), such as the cost of advertisement.

-   $i$ represents the observation number (e.g., different ad campaigns).

-   $w_j$​ are the weights (parameters) to be estimated using data.

-   $\hat{y}_i$​ is the predicted value, which is a linear combination of the weighted features.

This pure linear form implies that the model passes through the origin when all input features are 0.

In real-world scenarios, it is common to include a bias $w_0$ (or intercept) term in the model to account for the possibility that revenue is not strictly proportional to ad costs. For example, the company might have some baseline revenue even without any ad spending.

$$
\hat{y}_i = \sum_{j=1}^{n}w_jx_{ij} + w_o
$$

This form, which includes a bias term $w_0$, is technically an *affine* model. The pure linear form implies that the model passes through the origin when all input features are 0.

For simplicity, we can express the *affine* model as:

$$
\hat{y}_i = \sum_{j=0}^{n}w_jx_{ij} = w_0x_{i0} + w_1x_{i1} + \dots + w_nx_{in}
$$

where $x_{i0}​=1$, allowing the bias term to be treated as another weight in the model.

### A Simple Example: Revenue Proportional to Ad Cost

Consider a scenario where the revenue is directly proportional to the ad cost. This implies a simple linear relationship:

$y=wx$

Where $y$ is the revenue and $x$ is the ad cost. For example, if $y=12000$ and $x = 300$, the parameter $w$ (the constant of proportionality) can be calculated as:

$12000 = 300w \quad \Rightarrow \quad w = \frac{12000}{300} = 40$

This give the model $y = 40x$, meaning for every dollar spent on ads, the company generates 40 dollars in revenue.

This simple linear model would be the outcome if our algorithm were trained on a single example where the ad cost is \$300 and the corresponding revenue is \$12,000.

In practice, the model is typically trained on multiple data points to find the best-fitting $w$ value that generalizes well across different ad spend levels. The concept extends to more complex scenarios involving multiple features and the inclusion of the bias term, allowing the company to predict revenue and make informed decisions about ad spending.

### What is a Good Model?

In machine learning and statistical modeling, a **good model** balances the trade-offs between fitting the data well and generalizing to unseen data. Achieving this balance involves considering both the model\'s performance on the training data and its ability to generalize, which can be expressed through the **bias-variance trade-off**.

### Objective Function Overview

The objective function used in many models can be expressed as:

$\text{obj}(w) = L(w) + \Omega(w)$

Where:

-   $w$: Represents the model parameters (e.g., weights, coefficients).

-   $L(w)$: The training loss function (bias), which measures the error between predictions and actual values in the training set. It reflects how well the model fits the data.

-   $\Omega(w)$: The regularization term (variance), which controls the complexity of the model. Regularization helps avoid overfitting by penalizing overly complex models.

### Bias-Variance Trade-Off

A good model manages the trade-off between **bias** and **variance**:

-   **Bias**: Error due to overly simplistic assumptions in the model. High bias can cause underfitting, where the model fails to capture underlying patterns in the data.

-   **Variance**: Error due to sensitivity to small fluctuations in the training data. High variance can cause overfitting, where the model performs well on training data but poorly on unseen data.

### Criteria for Selecting a Model

Choosing the right model depends on several factors:

1.  **Training Loss (In-Sample Error)**: The error metric that evaluates the model's fit to the training data. A model should avoid high training loss (underfitting).

2.  **Regularization (Out-of-Sample Performance)**: Regularization helps gauge out-of-sample error by penalizing complex models that might overfit. A good model has low regularization error while fitting the data reasonably well.

3.  **Model Complexity**: Regularization focuses on model complexity. Simpler models are preferred if they achieve similar performance, as they are less likely to overfit and are more interpretable.

4.  **Speed and Efficiency**: Depending on the application, the model\'s training and testing speed may be a crucial factor. For real-time or large-scale applications, faster models might be necessary even if they sacrifice some accuracy.

5.  **Interpretability**: In some cases, understanding the model\'s decisions is important (e.g., in healthcare or finance). Simpler models like linear regression or decision trees are easier to interpret compared to complex models like deep neural networks.

6.  **Ease of Training and Tuning**: Some models require extensive hyperparameter tuning, which can be time-consuming and difficult. Simpler models might require less tuning and are easier to deploy.

A good model strikes a balance between underfitting and overfitting while meeting the practical requirements of speed, interpretability, and ease of training. The objective function\'s combination of training loss and regularization ensures that the model not only fits the data but also generalizes well to new data.
